...
def main():
    ...
    try:
        ...
        for event in [original,updates[0]]:
            response=checked(fixture['url'],'GET',f"rooms/{quote(fixture['rooms']['emma'],safe='')}/event/{quote(event['event_id'],safe='')}",token=fixture['users']['emma']['access_token']);content=response.get('content',{});assert not content.get('org.octosense.article') and not content.get('m.new_content',{}).get('org.octosense.article')
