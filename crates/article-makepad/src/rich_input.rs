//! Native rich input, adapted from Makepad TextInput (MIT), revision
//! 47837267faf6970a6cc36acedf9f83846b277307. Retains the native IME, clipboard,
//! selection and scrolling implementation. Article-specific formatted layout
//! uses the SAME LaidoutText for glyphs, selection, hit testing and the caret.
//! Upstream: https://github.com/kevinaboos/makepad/blob/47837267faf6970a6cc36acedf9f83846b277307/widgets/src/text_input.rs
use super::document::{Block, BlockKind, Mark};
use makepad_widgets::makepad_draw::shader::draw_text::FontFamily;
impl ArticleRichInput {
    fn restore_rich(&mut self, cx: &mut Cx, redo: bool) {
        let state = if redo {
            self.rich_redo.pop()
        } else {
            self.rich_undo.pop()
        };
        let Some((text, marks)) = state else { return };
        let current = (self.text.clone(), self.marks.clone());
        if redo {
            self.rich_undo.push(current)
        } else {
            self.rich_redo.push(current)
        }
        ...
        cx.widget_action(
            self.widget_uid(),
            ArticleRichInputAction::Changed(self.text.clone()),
        );
        self.redraw(cx);
    }
}
impl ArticleRichInputRef {
    ...
    pub fn apply_link(&self, cx: &mut Cx, url: Option<String>) -> Result<(), String> {
        let Some(mut inner) = self.borrow_mut() else {
            return Err("Select some text to format.".into());
        };
        let range = inner.selection.start().index..inner.selection.end().index;
        if range.is_empty() || inner.has_composition() {
            return Err("Select some text to format.".into());
        }
        let mut block = Block::new(BlockKind::Paragraph, inner.text.clone());
        block.marks = inner.marks.clone();
        block.format(range, None, None, Some(url))?;
        let previous = (inner.text.clone(), inner.marks.clone());
        inner.rich_undo.push(previous);
        inner.rich_redo.clear();
        inner.marks = block.marks;
        inner.laidout_text = None;
        inner.redraw(cx);
        Ok(())
    }
    pub fn set_block(&self, cx: &mut Cx, block: &Block, font_size: f32) {
        if let Some(mut inner) = self.borrow_mut() {
            ...
            if inner.base_font.is_none() {
                inner.base_font = Some(inner.draw_text.text_style.font_family.clone());
            }
            let family = if matches!(block.kind, BlockKind::Heading2 | BlockKind::Heading3) {
                inner.draw_bold.text_style.font_family.clone()
            } else {
                inner.base_font.clone().unwrap()
            };
            if inner.draw_text.text_style.font_family != family {
                inner.draw_text.text_style.font_family = family;
                inner.laidout_text = None;
            }
            if inner.marks != block.marks || inner.draw_text.text_style.font_size != font_size {
                inner.marks = block.marks.clone();
                inner.formatted_text = block.text.clone();
                inner.draw_text.text_style.font_size = font_size;
                inner.draw_bold.text_style.font_size = font_size;
                inner.draw_italic.text_style.font_size = font_size;
                inner.draw_bold_italic.text_style.font_size = font_size;
                inner.laidout_text = None;
                inner.redraw(cx);
            }
        }
    }
    pub fn content(&self) -> Option<(String, Vec<Mark>)> {
        self.borrow().map(|i| (i.text.clone(), i.marks.clone()))
    }
    pub fn toggle_format(&self, cx: &mut Cx, bold: bool) -> bool {
        let Some(mut inner) = self.borrow_mut() else {
            return false;
        };
        if inner.has_composition() {
            return false;
        }
        let range = inner.selection.start().index..inner.selection.end().index;
        if range.is_empty() {
        ...
        let mut block = Block::new(BlockKind::Paragraph, inner.text.clone());
        block.marks = inner.marks.clone();
        let flags = block.flags_at(range.start);
        if block
            .format(
                range,
        ...
        cx.widget_action(
            inner.widget_uid(),
            ArticleRichInputAction::Changed(inner.text.clone()),
        );
        true
    }
}
...
script_mod! {
    ...
    mod.widgets.ArticleRichInputBase = #(ArticleRichInput::register_widget(vm))

    /** The flat text field: an inset well with text, selection band and caret layers. */
    mod.widgets.ArticleRichInputFlat = set_type_default() do mod.widgets.ArticleRichInputBase{
        width: Fill
        height: Fit
    ...
    mod.widgets.ArticleRichInput = mod.widgets.ArticleRichInputFlat{
        draw_bg +: {
    ...
    mod.widgets.TextInputGradientX = mod.widgets.ArticleRichInput{
        draw_bg +: {
...
pub struct ArticleRichInput {
    ...
    #[rust]
    rich_redo: Vec<(String, Vec<Mark>)>,
    #[rust]
    formatted_text: String,
    #[rust]
    base_font: Option<FontFamily>,
    #[live]
    draw_link: DrawColor,
    #[live]
    draw_bold: DrawText,
...
impl ScriptHook for ArticleRichInput {
...
impl ArticleRichInput {
    fn emit_change(&mut self, cx: &mut Cx, uid: WidgetUid) {
        if self.formatted_text != self.text {
            self.rich_undo
                .push((self.formatted_text.clone(), self.marks.clone()));
            if self.rich_undo.len() > 100 {
                self.rich_undo.remove(0);
            }
            self.rich_redo.clear();
        }
        let mut block = Block::new(BlockKind::Paragraph, self.formatted_text.clone());
        block.marks = self.marks.clone();
        block.replace_text(self.text.clone());
        self.marks = block.marks;
        self.formatted_text = self.text.clone();
        cx.widget_action(uid, ArticleRichInputAction::Changed(self.text.clone()));
        if let Some(handler) = self.on_change.as_object() {
            ...
            cx.with_script_vm_id(vm_id, |vm| {
                ...
                vm.with_instruction_limit(200_000, |vm| {
    ...
    fn emit_return(&mut self, cx: &mut Cx, uid: WidgetUid, mods: KeyModifiers) {
        cx.widget_action(
            uid,
            ArticleRichInputAction::Returned(self.text.clone(), mods),
        );
    ...
    /// Enter. See the [`ArticleRichInput::submit_on_enter`] field for details.
    ...
    /// number: applying script to a ArticleRichInput resets its `#[live]` fields, and
    ...
    fn layout_text(&mut self, cx: &mut Cx2d) {
        ...
        self.laidout_width = max_width_in_lpxs;
        self.laidout_max_lines = self.draw_text.max_lines;
        self.laidout_text = Some(super::rich_layout::layout(
            cx,
            &self.draw_text,
            &self.draw_bold,
            &self.draw_italic,
            &self.draw_bold_italic,
            text,
            &self.marks,
            if wrap { max_width_in_lpxs } else { None },
        ));
    }
    ...
    fn scroll_to_cursor(&mut self, cx: &mut Cx2d, content_clip_index: usize) {
        ...
        // so that scrolling kicks in even when the ArticleRichInput's own walk height
        ...
        if self.is_multiline {
            ...
            let text_height = (laidout_text.size_in_lpxs.height * self.draw_text.font_scale) as f64;
            ...
            self.scroll_y = if centering.is_finite() {
                centering
            } else {
                0.0
            };
    ...
    pub fn replace_range(
        ...
        if self.has_composition() {
            ...
            let composition_start =
                floor_grapheme_boundary(&self.text, self.composition_start.min(len));
            let composition_end =
                floor_grapheme_boundary(&self.text, self.composition_end.min(len));
        ...
        let carry = |index: usize| {
            if index < start {
                ...
                floor_grapheme_boundary(
                    &self.text,
                    (index - (end - start) + replacement_len).min(self.text.len()),
                )
            } else {
    ...
    fn handle_focus_lost(&mut self, cx: &mut Cx, uid: WidgetUid) {
        ...
        cx.widget_action(uid, ArticleRichInputAction::KeyFocusLost);
    }
    ...
    fn draw_links(&mut self, cx: &mut Cx2d, text_rect: Rect) {
        let Some(layout) = &self.laidout_text else {
            return;
        };
        self.draw_link.color = vec4(0.34, 0.42, 0.58, 1.0);
        for mark in &self.marks {
            if mark.link.is_none() {
                continue;
            }
            let selection = Selection {
                anchor: Cursor {
            ...
            for SelectionRect { rect_in_lpxs, .. } in layout.selection_rects(selection) {
                let scale = self.draw_text.font_scale as f64;
                self.draw_link.draw_abs(
                    cx,
                    rect(
                        text_rect.pos.x + rect_in_lpxs.origin.x as f64 * scale,
                        text_rect.pos.y
                            + (rect_in_lpxs.origin.y + rect_in_lpxs.size.height) as f64 * scale
                            - 1.5,
                        rect_in_lpxs.size.width as f64 * scale,
                        1.0,
                    ),
                );
            }
    ...
    fn handle_navigation_key(&mut self, cx: &mut Cx, uid: WidgetUid, event: KeyEvent) -> bool {
        ...
        match result {
            Ok(true) => {}
            Ok(false) => cx.widget_action(uid, ArticleRichInputAction::KeyDownUnhandled(event)),
...
impl Widget for ArticleRichInput {
    fn script_call(
        &mut self,
        vm: &mut ScriptVm,
        method: LiveId,
    ...
    fn set_text(&mut self, cx: &mut Cx, text: &str) {
        self.marks.clear();
        self.formatted_text = text.to_owned();
    ...
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, _scope: &mut Scope) {
        if let Event::KeyDown(key) = event {
            if cx.has_key_focus(self.draw_bg.area())
                ...
                && !self.has_composition()
            {
                self.restore_rich(cx, key.modifiers.shift);
                return;
            }
        }
        ...
        match event.hits(cx, self.draw_bg.area()) {
            ...
            Hit::KeyFocus(_) => {
                ...
                self.last_sent_ime_sel_start = self.selection.start().index;
                self.last_sent_ime_sel_end = self.selection.end().index;
                cx.widget_action(uid, ArticleRichInputAction::KeyFocus);
            }
            Hit::KeyFocusLost(_) => {
                self.handle_focus_lost(cx, uid);
            }
            ...
            Hit::KeyDown(KeyEvent {
                ...
                cx.widget_action(uid, ArticleRichInputAction::Escaped);
            }
            Hit::KeyDown(KeyEvent {
            ...
            Hit::KeyDown(event) => {
                cx.widget_action(uid, ArticleRichInputAction::KeyDownUnhandled(event));
            }
            _ => {}
        }
    }
...
impl ArticleRichInputRef {
    /// See [`ArticleRichInput::set_max_lines`].
    ...
    /// See [`ArticleRichInput::take_key_focus`].
    ...
    /// See [`ArticleRichInput::set_height`].
    ...
    /// See [`ArticleRichInput::scroll_to_top`].
    ...
    /// Returns whether this (multiline) text input emits `Returned` on plain
    /// Enter. See [`ArticleRichInput::submit_on_enter`] for details.
    pub fn submit_on_enter(&self) -> bool {
        self.borrow()
    ...
    pub fn cursor_rect_in_absolute(&self, cx: &Cx) -> Option<Rect> {
        self.borrow()
            .and_then(|inner| inner.cursor_rect_in_absolute(cx))
    }
    ...
    pub fn returned(&self, actions: &Actions) -> Option<(String, KeyModifiers)> {
        for action in
            actions.filter_widget_actions_cast::<ArticleRichInputAction>(self.widget_uid())
        {
            if let ArticleRichInputAction::Returned(text, modifiers) = action {
    ...
    pub fn escaped(&self, actions: &Actions) -> bool {
        for action in
            actions.filter_widget_actions_cast::<ArticleRichInputAction>(self.widget_uid())
        {
            if let ArticleRichInputAction::Escaped = action {
                return true;
            }
        }
        false
    ...
    /// Returns true if this ArticleRichInput lost keyboard focus in the given actions.
    pub fn key_focus_lost(&self, actions: &Actions) -> bool {
        for action in
            actions.filter_widget_actions_cast::<ArticleRichInputAction>(self.widget_uid())
        {
            if let ArticleRichInputAction::KeyFocusLost = action {
                return true;
            }
        }
        false
    ...
    pub fn changed(&self, actions: &Actions) -> Option<String> {
        for action in
            actions.filter_widget_actions_cast::<ArticleRichInputAction>(self.widget_uid())
        {
            if let ArticleRichInputAction::Changed(text) = action {
                return Some(text);
            }
        }
        None
    ...
    pub fn key_down_unhandled(&self, actions: &Actions) -> Option<KeyEvent> {
        for action in
            actions.filter_widget_actions_cast::<ArticleRichInputAction>(self.widget_uid())
        {
            if let ArticleRichInputAction::KeyDownUnhandled(event) = action {
                return Some(event);
            }
        }
        None
    ...
    /// to a new `ArticleRichInputState` object.
    pub fn save_state(&self) -> ArticleRichInputState {
        if let Some(inner) = self.borrow() {
            ArticleRichInputState {
            ...
            ArticleRichInputState::default()
        }
    }

    /// Restores the internal state of this text input widget
    /// from the given `ArticleRichInputState` object.
    pub fn restore_state(&self, cx: &mut Cx, state: ArticleRichInputState) {
        if let Some(mut inner) = self.borrow_mut() {
    ...
    /// See [`ArticleRichInput::replace_range`].
    pub fn replace_range(
        &self,
        cx: &mut Cx,
        range: Range<usize>,
    ...
    /// See [`ArticleRichInput::is_composing`].
    pub fn is_composing(&self) -> bool {
        self.borrow().map_or(false, |inner| inner.is_composing())
    }

    /// See [`ArticleRichInput::force_new_edit_group`].
...
/// How an edit made through [`ArticleRichInput::replace_range`] joins the undo history.
...
/// Why [`ArticleRichInput::replace_range`] refused an edit. The text is untouched in every case.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ReplaceRangeError {
    ...
    /// the IME's until it commits; wait for [`ArticleRichInput::is_composing`] to clear.
    Composing,
}

/// The saved (checkpointed) state of a text input widget.
#[derive(Clone, Debug, Default)]
pub struct ArticleRichInputState {
    text: String,
    password_text: String,
    selection: Selection,
    history: History,
...
#[derive(Clone, Debug, Default)]
pub enum ArticleRichInputAction {
    #[default]
    None,
    KeyFocus,
    KeyFocusLost,
